...
export class RequestsController {
  ...
  async reject(@CurrentUser() user: any, @Param('id') id: string, @Body() dto: RejectRequestDto) {
    return this.requests.reject(user.tenantId, id, user.id, dto.reason);
  }

  @Post(':id/apply')
  @ApiOperation({ summary: 'Apply approved request' })
  async apply(@CurrentUser() user: any, @Param('id') id: string) {
    return this.requests.apply(user.tenantId, id, user.id);
  }

  @Post(':id/comments')
  @ApiOperation({ summary: 'Add comment to request' })
  async addComment(@CurrentUser() user: any, @Param('id') id: string, @Body() dto: AddCommentDto) {
