...
export class CalendarService {
  ...
  async generateIcs(tenantId: string) {
    const events = await this.prisma.calendarEvent.findMany({ where: { tenantId }, orderBy: { startAt: 'asc' } });
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
  ...
  async getBusinessHours(tenantId: string) {
    const rows = await this.prisma.businessHours.findMany({ where: { tenantId }, orderBy: { weekday: 'asc' } });
    return { hours: rows };
  }
  ...
  async listHolidays(tenantId: string) {
    const rows = await this.prisma.holiday.findMany({ where: { tenantId }, orderBy: { date: 'asc' } });
    return { holidays: rows };
  }

  async addHoliday(tenantId: string, dto: { date: string; name: string }) {
    const h = await this.prisma.holiday.create({ data: { tenantId, date: new Date(dto.date), name: dto.name } });
    return h;
  }
  ...
  async saveOAuth(tenantId: string, provider: string, credentialsJson: any) {
    const enc = new (require('../crypto').EncryptionService)(process.env.MASTER_KEY_HEX || ''.padEnd(64,'0'));
    const payload = enc.encrypt(JSON.stringify(credentialsJson));
    const up = await this.prisma.oAuthCredential.upsert({
